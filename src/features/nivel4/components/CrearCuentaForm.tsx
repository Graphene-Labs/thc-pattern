  ...
  return (
    <form onSubmit={e => { e.preventDefault(); registrar() }} className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <label htmlFor="n4-nombre" className="text-sm font-medium text-gray-700">Nombre</label>
        <input id="n4-nombre" value={formData.nombre} onChange={e => updateField('nombre', e.target.value)}
      ...
      <div className="flex flex-col gap-1">
        <label htmlFor="n4-email" className="text-sm font-medium text-gray-700">Email</label>
        <input id="n4-email" type="email" value={formData.email} onChange={e => updateField('email', e.target.value)}
          placeholder="tu@email.com" disabled={isLoading}
          className="rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm outline-none focus:ring-2 focus:ring-violet-500 focus:border-violet-500 disabled:bg-gray-50" />
        {errors.email && <p role="alert" className="text-xs text-red-500">⚠ {errors.email}</p>}
      </div>
      <div className="flex flex-col gap-1">
        <label htmlFor="n4-password" className="text-sm font-medium text-gray-700">Contraseña</label>
        <input id="n4-password" type="password" value={formData.password} onChange={e => updateField('password', e.target.value)}
